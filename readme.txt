...
/*         
    ...
    handleChangeUpload = (e) => {
        ...
        if(e.target.files[0] && (e.target.files[0].type === 'image/png' || e.target.files[0].type === 'image/gif' || e.target.files[0].type === 'image/jpeg')){
    ...
    imgData = () => {
        ...
        if(this.state.fileError){
            showMessage = <div className="col-md-4 m-auto text-center"><i className="far fa-file-image"></i><span>{this.state.fileError}</span></div>
            
        } else if(this.state.file) {
            showMessage = <div className="col-md-4 m-auto text-center"><img className="img-fluid mt-3" src={this.state.file} alt="preview owner img"/><span>Your selected image</span></div>
        } else {
            showMessage = <div className="col-md-3 m-auto text-center"><i className="fas fa-camera-retro"></i><span>Preview image here</span></div>  
        }
               ...
               <div className="card p-4">
                ...
                <h5 className="col-md-12 text-center m-3">Upload owner images</h5>  
                    <div className="col-md-12 custom-control custom-checkbox">
                        <input type="file" className="custom-file-input" id="customFile" onChange= {this.handleChangeUpload } name="file" accept="image/x-png,image/gif,image/jpeg" multiple required/>
